                ...
                switch(increment){
                    case 0:
                        if(button_clicked == '#' || i == 127){
                            ...
                            password[i++] = button_clicked;
                        }
                        break;
                    case 1:
                        if(j < i){
                            uart0_putc(password[j++]);
                        }else{
                            j = 0;
                            i = 0;
                            increment++;
...
int main(void)
{
   ...
   // xTaskCreate(switch_task, "switch",USERTASK_STACK_SIZE, NULL, LOW_PRIO, NULL);
    ...
    xTaskCreate( PasswordTask, "Password", USERTASK_STACK_SIZE, NULL, LOW_PRIO, NULL);
    vTaskStartScheduler();
	return 0;
}
