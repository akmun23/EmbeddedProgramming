...
INT8U get_keyboard()
{
    ...
    if(uxQueueMessagesWaiting(xQueue_key)){
        if( xSemaphoreTake( xSemaphore_key, portMAX_DELAY)){ //( TickType_t ) 100 ) == pdTRUE )
            if( xQueueReceive( xQueue_key, &ch, portMAX_DELAY )){
...
BOOLEAN check_column(INT8U x)
{
    ...
    if( xSemaphoreTake( xSemaphore_key, portMAX_DELAY)){ //( TickType_t ) 100 ) == pdTRUE )
        ...
        {                                               // ...we first find the row number with the function row()
            ...
            xSemaphoreGive( xSemaphore_key );
            return( xQueueSend( xQueue_key, &button, portMAX_DELAY));
        }
        xSemaphoreGive( xSemaphore_key );
    }
